...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
    ...
    if (tf == NULL) {
        panic("sys_env_set_trapframe: tf is NULL\n");
    }
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uint32_t) srcva < UTOP) {
		if (srcva != (void*) ROUNDDOWN((uint32_t)srcva, PGSIZE)) {
			return -E_INVAL;
		}
		if (!(perm & (PTE_U | PTE_P))) {
			return -E_INVAL;
		...
		if ((uint32_t) recv->env_ipc_dstva < UTOP) {
			recv->env_ipc_perm = perm;
			r = page_insert(recv->env_pgdir, pp, recv->env_ipc_dstva, perm);
			if (r < 0) {
				return r;  // -E_NO_MEM
			}
		}
...
sys_ipc_recv(void *dstva)
{
	...
	if ((uint32_t) dstva < UTOP) {
		if (dstva != (void*) ROUNDDOWN((uint32_t)dstva, PGSIZE)) {
			return -E_INVAL;
		}
	}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_cgetc:
		...
		break;
	case SYS_getenvid:
		res = (int32_t) sys_getenvid();
		break;
	case SYS_env_destroy:
		res = (int32_t) sys_env_destroy((envid_t) a1);
		break;
	case SYS_yield:
		sys_yield();
		res = 0;
		break;
	case SYS_exofork:
		res = (int32_t) sys_exofork();
		break;
	case SYS_env_set_status:
		res = (int32_t) sys_env_set_status((envid_t) a1, (int) a2);
		break;
    case SYS_env_set_trapframe:
		res = (int32_t) sys_env_set_trapframe((envid_t) a1, (struct Trapframe*) a2);
		break;
	case SYS_page_alloc:
		res = (int32_t) sys_page_alloc((envid_t) a1, (void*) a2, (int) a3);
		break;
	case SYS_page_map:
		res = (int32_t) sys_page_map((envid_t) a1, (void*) a2, (envid_t) a3, (void*) a4, (int) a5);
		break;
	case SYS_page_unmap:
		res = (int32_t) sys_page_unmap((envid_t) a1, (void*) a2);
		break;
	case SYS_env_set_pgfault_upcall:
		res = (int32_t) sys_env_set_pgfault_upcall((envid_t) a1, (void *) a2);
		break;
	case SYS_ipc_try_send:
		res = (int32_t) sys_ipc_try_send((envid_t) a1, a2, (void*) a3, (unsigned) a4);
		break;
	case SYS_ipc_recv:
		res = (int32_t) sys_ipc_recv((void*) a1);
		break;
	default:
		return -E_INVAL;
	}
