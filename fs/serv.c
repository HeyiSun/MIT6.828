...
struct OpenFile {
	uint32_t o_fileid;	// file id, 按顺序从0开始到MAXOPEN - 1
...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    req->req_n = (req->req_n < PGSIZE) ? req->req_n : PGSIZE;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    req->req_n = (req->req_n < sizeof(req->req_buf)) ? req->req_n : sizeof(req->req_buf);
